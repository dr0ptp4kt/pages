...
class MusicSync {
  ...
  async loadAlbum(url, audioBaseUrl) {
    try {
      ...
      this.tracks = (this.albumData.tracks || []).map((t, i) => ({
        ...
        notesFileV4: t.notes_file_v4 || null,
      }));
  ...
  async loadTrackEvents(trackIndex) {
    ...
    // Try v4 file first (has midi_source per note for provenance display)
    const notesFile = track.notesFileV4 || track.file;
    try {
      let resp = await fetch(this.baseUrl + notesFile);
      if (!resp.ok && notesFile !== track.file) {
        // Fall back to original notes file
        resp = await fetch(this.baseUrl + track.file);
      }
      ...
      if (data.legend && Array.isArray(data.events) && Array.isArray(data.events[0])) {
        ...
        legend = { instruments: {}, fields: ['t', 'dur', 'note', 'inst', 'vel', 'ch', 'midi_source'] };
        const instMap = {};
        let idx = 0;
        const raw = data.events || [];
        // Also store MIDI sources list for the track
        if (data.midi_sources) {
          legend.midi_sources = data.midi_sources;
        }
        events = raw.map(ev => {
          ...
          return [ev.t, ev.dur, ev.note, instMap[inst], ev.vel, ev.ch || 0, ev.midi_source || ''];
        });
      }
