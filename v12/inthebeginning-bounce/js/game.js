...
class Game {
  ...
  setGridDim(dim) {
    ...
    // In game mode, 2D/3D toggle also changes gameplay perspective
    if (this.mode === 'game') {
      const want3D = dim === '3d';
      this.renderer3d.disabled3D = !want3D;
      this._user3DOverride = true; // Prevent auto-switch from overriding
      // Toggle obstacle spawn direction
      if (this.obstacles) {
        for (const obs of this.obstacles.obstacles) {
          obs.horizontal = !want3D;
        }
      }
    }
  }
  ...
  setIntensity(intensity) {
    if (this.mode === 'game') {
      ...
      // 3D mode: obstacles fly through (with fade-out), slightly slower for readability
      // Mobile gets reduced speed since touch controls are less responsive
      const is3D = this.gridDim === '3d';
      const isMobile = this.width < 768;
      const baseFall = is3D ? (isMobile ? 150 : 180) : 140;
      this.fallSpeed = (baseFall + intensity * 60) * this.userSpeedMult * levelScale;
    } else {
      this.speed = 0.3;
      this.scrollSpeed = 30;
      this.fallSpeed = 30;
