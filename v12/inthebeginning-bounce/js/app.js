...
class CosmicRunnerApp {
  ...
  async init() {
    ...
    if (speedDown) speedDown.addEventListener('click', () => {
      ...
      // Speed buttons affect gameplay only, not music playback
    });
  ...
  _applyMutation(index) {
    ...
    if (!mutation) return;
    if (this.player) {
      // Stop all currently sounding notes to prevent cacophony buildup
      // when rapidly switching mutations (especially in Synth mode)
      if (this.player._synth) this.player._synth.stopAll();
  ...
  async _initSoundMode() {
    switch (this.soundMode) {
      ...
      case 'synth':
        ...
        await this.player.play();
        // Set initial HUD track name
        const synthTrackName = this.player.musicGenerator.getCurrentTrackName() || 'Synth';
        if (this.hudTrack) this.hudTrack.textContent = synthTrackName;
        if (this.songTitleDisplay) this.songTitleDisplay.textContent = synthTrackName;
        break;
      case 'wasm': {
  ...
  _switchMode(mode) {
    ...
    // Mode-aware HUD: hide buttons based on display mode and sound mode
    const isPlayer = mode === 'player';
    const isMp3 = this.soundMode === 'mp3';
    // Speed buttons: hide in player mode
    ['speed-down', 'speed-up'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.style.display = isPlayer ? 'none' : '';
    });
    // Mutation + style: only show for MIDI/Synth/WASM (not MP3), hide in player mode
    ['mutation-btn', 'style-btn'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.style.display = (isPlayer || isMp3) ? 'none' : '';
    });
    // Instrument soundbank: only for MIDI/Synth/WASM
    const instrBtn = document.getElementById('instrument-btn');
    if (instrBtn) instrBtn.style.display = isMp3 ? 'none' : '';
