 ...
 *
 * Upgraded to use SynthEngine for additive synthesis with 16+ timbres.
 * Uses Web Worker for non-blocking MIDI parsing.
...
class MidiPlayer {
  constructor() {
    ...
    // Initialize synth engine and worker
    this._initSynth();
    this._initWorker();
  }

  _initSynth() {
    if (typeof SynthEngine !== 'undefined') {
  ...
  async loadMidi(buffer) {
    ...
    try {
      ...
      if (this._worker && this._workerReady) {
        ...
        result = MidiPlayer._parseMidi(data);
  ...
  getCurrentTime() {
    ...
    return this._synth.ctx.currentTime - this._startCtxTime;
  }
  ...
  _scheduleLoop() {
    ...
    while (this._nextNote < this._notes.length) {
      ...
      if (noteTime >= now - 0.05) {
        if (this._synth) {
          this._synth.playNote(note, noteTime - now);
        }
      }
      this._nextNote++;
    }
  ...
  // ──── Fallback MIDI Parser (main thread) ────

  /**
   * Parse a Standard MIDI File on the main thread (fallback).
